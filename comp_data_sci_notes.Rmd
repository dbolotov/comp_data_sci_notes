...
## EDA examples - TODO
### Springleaf competition EDA I - TODO
### Springleaf competition EDA II - TODO
### Numerai competition EDA - TODO
...
### Expedia challenge
#### Problem statement
- Predict which hotel group a user is going to book based on customer behavior.
- Data includes what customers searched for, how they interacted with the search results (clicks or books), and whether or not the search result was a travel package.

#### Data leak
- The competition had a non-trivial and extremely hard to exploit data leak.
- The feature destination_distance is the distance from a user city to an actual hotel the user clicked or booked. The prediction target is a characteristic of an actual hotel. This feature is very precise, so the destination_distance - user_city pair corresponded to unique hotels.
- Can treat this pair as a proxy to the target. When encountering this pair in the test set, we can take the train set label as the prediction. This worked nearly perfectly for pairs present in both train and test.
- Nearly half of the test set consisted of new pairs. It was possible to improve the results using two different ways:
    - 1. Create features based on counts on corteges similar to destination_distance - user_city pair. E.g., how many hotels of each group for the (user_city, hotel_country, hotel_city) triplet?
    - 2. Find more matches by finding true coordinates of user cities and hotel cities. This, combined with the destination_distance - user_city pairs, it was possible to find good approximations for coordinates of hotels.

#### Calculating distances
- Working with geographical coordinates, the distances are geodesic, and are calculated via Haversine formula.

##### Hotel cities algorithm - version 1
- Suppose we know the coordinates of 3 points, and distances from a 4th point with unknown coordinates to each of the 3 points. Solving a system of 3 equations (one for each distance) gives true coordinates for the 4th point.
- At first, get the coordinates of 3 cities. Then, iteratively compute distances to each new unknown city.
- This led to some cities being located in oceans, meaning that this algorithm is not very precise. A rounding error accumulates after every iteration.

##### Hotel cities algorithm - version 2
- Instead of using 3 equations to compute the coordinates of an unknown, create a giant system of equations from all known distances, with true coordinates being the unknown variables. This results in a system of 100000s of equations and 10000s of unknown variables, but is very sparse.
- Special methods from scipy can be applied to efficiently solve such a system.
- This leads to very precise coordinates for hotel and user cities.

##### Feature engineering
- Using city coordinates and destination distance it's possible to find an approximation of true coordinates of an actual hotel.
- When we fix user_city and draw a circumference around it with radius = destination_distance, the true location must be somewhere on that circumference.
- Fix some hotel_city, and draw circumferences from all user cities to that hotel for every given destination distance.
- A city contains a limited number of hotels. Hotels are on the intersection points. The more circumferences intersect in such points, the higher the probability of a hotel being in that point.
- The plots produced from the circumferences are messy, but contain clusters of points that can be of use.
- Using the clusters:
    - For every city, create a grid around its center (e.g., 10km x 10km, with step size of 100m).
    - Using training data, for every cell in a grid, count how many hotels of each type is present there. If a circumference goes through a cell, we give +1 to the hotel type corresponding to that circumference.
    - During inference, we also draw a circumference based on destination distance feature. We see through which grid cells it went through, and use information from those cells to create features (sum of all counters, avg of all counters, max, etc.).

##### Final model
- All the features directly use the target variable. We cannot use them as-is in training, and should generate them in OOF fashion for train data.
- We had training data for years 2013 and 2014. To generate features for 2014, use labeled data from 2013, and vice versa.
- For test features (2015), use all training data.
- All the features were put into xgboost for 16 hours of training.

##### Results
- This solution was #4 on the leaderboard.
- The winning solution had a much higher score than other solutions. It used similar methods, but was able to extract way more signal.
...
- Split all params to be tuned into two groups (color-coded):
    - 1. <span style="color:red">Parameters that constrain the model (red color)</span>. If we increase these params, the model changes its behavior from overfitting to underfitting. Larger param values mean heavier constraint. Decrease these params to allow model to fit easier.
    - 2. <span style="color:green">Parameters that lead to overfit (green color)</span>. Increasing these params lead to a better fit (overfit) on train set. Increase these params if model is underfitting; decrease if overfitting.
...
- Sort all parameters by these principles:
    - Importance (tune the params in this order)
    - Feasibility (easy to tune to hard to tune)
    - Understanding (easy to understand to hard to understand)

#### Dmitry Altukhov's tips
##### Data loading 
- Do basic preprocessing and convert csv/txt files into hdf5/npy for much faster loading.
    - save to hdf5/npy after doing label encoding, etc
- Do not forget that by default data is stored in 64-bit arrays (numpy/pandas), most of the time you can safely downcast to 32-bits.
- Large datasets can be processed in chunks (in pandas).
    - The entire dataset does not have to be loaded into memory.

##### Performance evaluation
- Even for medium-size datasets (50000-100000 rows), can validate with simple train/test loop.
- Switch to full cv when it is really needed - when hit performance limit and can only move ahead with marginal improvements.
- Start with fastest models (e.g., LightGBM). Use early stopping. Switch to tuning models, sampling and stacking only after finishing feature engineering.

##### Fast and dirty always better
- Focus on what's really important - the data. EDA & domain specific knowledge is more important than creating unnecessary classes and personal frameworks.
- If you feel uncomfortable with given computational resources, just rent a larger server

#### Mikhail Trofimov's tips
##### Initial pipeline
- Start with simple (or even primitive) solution.
- The purpose of this is to debug the full pipeline, from reading data to writing submission file.
- Start with random forest rather than GBDT, since RF requires almost no tuning.
...
##### Read papers related to the competition
- This can get you ideas about ML-related topics.
- And get familiar with the problem domain.

#### Dmitry Ulyanov's tips

##### My pipeline
- Read forums and examine kernels first.
    - Someone might have found a bug, and the competition has changed.
- Start with EDA and a baseline.
    - To make sure the data is loaded correctly.
    - To check if validation is stable.
- Generate features in bulk.
    - At start, create all the features you can make up.
    - Evaluate many features at once (not "add one and evaluate")
- Hyperparameters optimization
    - First find the params to overfit train dataset.
    - And then try to constrain the model.

##### Code organization: keeping it clean
- Very important to have reproducible results. Keep important code clean.
- Long execution history leads to mistakes. E.g., too many global variables.
    - Restart the notebook sometimes.
- Use one notebook per submission, and use git.
- Before a submission, restart the kernel (use restart and run all button).

##### Conde organization: test/val
- Convenient way to validate the models that allows to use validation code with minimal changes to retrain a model on the whole dataset.
    - Split *train.csv* into *train* and *val* with structure of *train.csv* and *test.csv*.
        ...
        # when validating, set this at the top of notebook:
        train_path = 'data/val/train.csv'
        test_path = 'data/val/val.csv'
        
        # when retraining on whole dataset:
        train_path = 'data/train.csv'
        test_path = 'data/test.csv'
        ```

##### Code organization: use macros

##### Code organization: custom library
- create a custom library with code that always gets repeated.
...
- Feature engineering (until last 3-4 days)
    - Work individually for 1 week, then read forums/kernels
- Modelling (until last 3-4 days)
- Ensembling (last 3-4 days)

#### Understand broadly the problem
- Understand the type of problem (image classification, text, optimization, etc.)
- How BIG is the data?
- Hardware needed (CPUs, GPUs, RAM, Disk space)
- Software needed (TF, sklearn, LightGBM, XGBoost)
- What is the metric used in the competition?
    - Find solutions from past similar competitions.

#### Exploratory data analysis
- Plost histograms of variables. Chack that a feature looks similar between train and test.
    - Maybe these features need to be scaled or removed.
- Plot features vs target and vs time if available.
- Consider univariate predictability metrics (Information Value, R, AUC).
- Bin numerical features and construct correlation matrices: this helps to notice strange relationships.
...
- If data has time series, do **time-based validation**. Invervals should be similar too (e.g., if test data is 3 months into the future, split the train data into train and val 3 months ahead).
- Check for different entities between train and test. E.g., if train and test sets have different customers, make sure that train and val has different customers. This is **stratified validation**.
- Check that the train and test are completely random. Then do **random validation** (random K-fold).
- **Combination of all the above**. E.g., a time-series data with different entities and/or random elements.
- Use the test LB to test: check if improvements on val set lead to improvements on the LB.

#### Feature engineering
- The type of problem defines the feature engineering. The following is a list of possible features, and example competitions where they were used.
    - **Image classification**: Scaling, shifting, rotations, CNNs. Previous Data Science Bowls.
    - **Sound classification**: Fourier, MFCC, Specgrams, scaling. Tensor Flow Speech Recognition.
    ...
    - **Categorical**: Target enc, freq, one-hot, ordinal, label encoding. Amazon Employee Access.
    - **Numerical**: Scaling, binning, derivatives, outlier removals, dimensionality reduction. Africa Soil.
    - **Intercations**: Multiplications, divisions, group-by features, concatenations. Homesite
    - **Recommenders**: Transactional history, item popularity, frequency of purchase. Acquire Valued Shoppers.
...
- **Sound classification**: CNNs (RNN), LSTM
- **Text classification**: GBM, Linear, DL, Naive bayes, KNNs, LibFM, LIBFFM
- **Time series**: Autoregressive models, ARIMA, linear, GBMs, DL, LSTMs
- **Categorical fatures**:  GBMs, Linear models, DL, LibFM, LIBFFM
- **Numerical features**: GBMs, Linear models, DL, SVMs
- **Interactions**: GBMs, Linear models, DL
- **Recommenders**: CF, DL, LibFM, LIBFFM, GBMs

#### Ensembling
- All this time, predictions on interval validation and test are saved.
    - If collaborating with others, this is the point where everyone passes on their predictions as .csv files.
- There are different ways to combine, from averaging to multilayer stacking.
...
- The stacking process repeats the modeling process.
    - Can do feature engineering, create and remove features for stacked models.
...
- You score better.
    ...
    - Every participant has a different angle, leading to more through solutions.
- Start collaborating after getting some experience (2-3 competitions) to understand the dynamics.
- Start with people around your "rank", and grow together.
...
- Monitor correlations. If correlations are too high and submissions exist with high scores but significantly lower correlations, they could be considered too.
...
- Take a break often to rest your mind - do some physical exercise as it is unhealthy sitting in a chair for many hours.
- The kaggle community may be the most kind, helpful community.
- After the competition, lok for people sharing approaches.
...
### Statistics and distance based fatures
- Example: data for CTR task, consisting of User_id, Page_id, Ad_price, and Ad_position.
- The most straightforward way to solve this problem is to label-encode Ad_position and fit a classifier. This model would be very good, and take into account hidden relations between variables. But no matter how good it is, it still treats all the data points independently. This is where feature engineering can be applied.
- We can imply that an ad with the lowest price on the page will catch most of the attention, and the rest of the ads won't be very attractive. It is easy to calculate the features relevant to such an implication: we can add lowest and highest ad prices for every user and page pair, and add position of the ad with the lowest price.
...
- More useful features:
    - How many pages user visited total and during session
...
### Neighbors
- This techique is useful if there are no features to use `groupby` on.
- Replace the grouping operation with finding nearest neighbors.
- This approach is more flexible, but much harder to implement.
- The most natural example of neighborhood analysis arises from property pricing.
    - Regular features would be floor space, number of rooms, presence of a bus stop, etc.
    - But to create a really good model, use the following features:
        - Number of houses in 500m, 1000m radius.
        - Average price per square meter in 500m, 1000m radius.
        - Number of schools/supermarkets/parking lots in 500m, 1000m.
        - Distance to closest subway station, gyms, etc.
- Apply this method to abstract or anonymized feature spaces. 
- Example from SpringLeaf competition:
    - Mean code all variables. This creates a homogeneous feature space, so no need to worry about scaling and importance of each particular feature.
    - For every point, find 2000 nearest neighbors using Bray-Curtis metric: $\sum|u_i-v_i|/\sum|u_i+v_i|$.
    - Calculate various features from those 2000 neighbors:
        ...
        - Mean distance to 10 closest neighbors with target 1, target 0

### Matrix factorizations

#### What is matrix factorization?
- Example: movie recommendation based on user and movie data and user ratings.
- Create a user-item matrix with each row corresponding to a user and each column corresponding to a movie. Each cell (i,j) stores a rating from user i for item j.
- Assume that ith user has features $U_i$ and jth movie has features $M_i$. $R_{ij}$ is the scalar product of these features per user rating. We can apply matrix factorization for learning the features for items and users.
- Sometimes, features can have an interpretation. E.g., the first feature in item can be a measure of movie sadness. Generally, consider these as abstract features which we can use to encode a user as we did before, with label encoding or one-hot encoding.
- Specifically, our assumption about scalar product is the following: If we represent all attributes of users and items as matrices, their matrix product will be very close to the matrix with ratings. In other words, we try to find matrices $U$ and $M$ such that their product gives the matrix $R$. That is why this approach is called matrix factorization or matrix decomposition.

#### Documents/words example
- Sometimes, we need only features corresponding to rows.
- Example: text processing. Extract bag of words features, and each document is described by a large sparse vector (row with 0 and 1 for each word).
- If we do MF over these sparse features, we will get a representation for docs and a representation for terms. Now, every document is described by a small, dense vector. These are now features to be used in model training.
- This approach is often called *dimensionality reduction*. It is an efficient way to reduce the size of the feature matrix and extract real-valued features from one-hot encoded categorical ones.
- In competitions, we often have different options for pre-processing: vanilla BOW, BOW+TFIDF, BOW(bigrams), etc. Using MF, we can extract features from all these matrices. Since the resulting matrices will be small, we can easily join them, and use them together with tree-based models.

#### Notes about matrix factorization
- We are not constrained to use the whole matrix: can apply MF to a subset of columns, and leave the others as-is.
- Besides dimension reduction, can use MF to provide another representation of the same data. This is especially useful for ensembles.
- MF is a lossy transformation. Its efficiency depends on the particular task and the number of latent factors (usually 5-100). The number of latent factors is a hyperparameter that should be tuned.
  
#### Implementation
- There are several MF methods in sklearn.
- SVD and PCA: standard tools for MF
...
- Non-negative Matrix Factorization (NMF): ensures that all latent factors are non-negative; good for counts-like data.
    - NMF has an iteresting property - it transforms data in a way that makes data more suitable for decision trees. NMF-transofmred data forms lines parallel to axes.
- MF is similar in spirit to linear models. Therefore, can use the same transformation tricks. E.g., apply MF to logarithmically-transformed data.

#### Gotchas
- MF is a trainable transformation and has its own params. Be careful and use the same transformation for all parts of your dataset.
- Wrong way:
    ```
...
#### Summary
- MF is a very general approach for dimensionality reduction and feature extraction.
- Can be used for transforming categorical features into real-valued.
- Many tricks suitable for linear models can be useful for MF.

### Feature interactions
- Example: the best advertisement banner to display on a website. Data has category_ad (what the ad is showing), category_site (the kind of site that ad is shown on), and is_clicked.
- The combination of category_ad and category_site is really important. 
- We can explicitly construct this combination in a new feature, ad_site.
    - E.g., "auto_part" and "game_news" becomes "auto_part|game_news".
    - There are two ways of constructing this new feature:
        - 1. Concatenate the features f1 and f2 to create a new categorical feature f_join, then apply one-hot encoding to f_join.
        - 2. Apply one-hot encoding to features f1 and f2. Then, construct new metrics by multiplying each column from f1 encoded to f2 encoded.
        - Both methods result in practically the same feature representation.
- Similar ideas can be applied to real-valued features. Use any function that takes two arguments: product, sum, diff, division.
- These transformations significantly enlarge feature space and make learning easier. But it makes overfitting easier too.
- For tree-based models, it is difficult to extract such dependencies. The above transformations are very effective in this case.

### Practical notes
- There are a lot of possible pairwise interactions - N*N for N features. This is even more severe if several types of interactions are used.
- To moderate this, use dimensionality reduction or feature selection. Recommendation: use feature selection, since not all but only a few interactions often achieve the same quality as all features.

### Example of interaction generation pipeline
- Construct all pairwise feature interactions for each type (sum, diff, etc.).
- Fit random forests over each set of features.
- Select a few most important features for each type based on the feature importance from RF.
- Add these features to the original data.
- Use this new dataset as input to a learning algorithm.

### Interactions' order
- The pairwise approach can be generalized to higher orders.
- The number of features grows rapidly, and high-order interactions are constructed semi-manually.

### Extracting features from decision trees
- This method is used to construct categorical features from decision trees, and is a powerful way to extract high-order interactions.
- Each leaf is mapped into a binary feature. The index of the object's leaf can be used as a value for a new categorical feature.
- If a tree ensemble is used, then this can be applied to each of the trees.
- Implementation in sklearn:
    ```
      tree_model.apply() # take as input the feature matrix, and return corresponding leaf indices
    ```
...
### t-SNE
- This is an example of non-linear dimensionality reduction, generally called manifold learning.
- t-SNE tries to projects points from a high-dimensional space into smaller dimensions such that the distances between points are approximately preserved.
- Example: MNIST dataset. Can project points from 28x28-D space onto 2-D space. This projection forms explicit clusters that correspond to the target class. Moreover, neighboring clusters correspond to visually similar numbers.
- If data has explicit structure, it is likely to be reflected on t-SNE plots, so t-SNE is widely used in EDA.
- t-SNE results strongly depend on the parameters, and interpretation of the results is not a simple task.
    - A bad choice of hyperparams (perplexity) may lead to bad t-SNE results.
    - One cannot infer the size of the original clusters using the size of projected clusters. Same for distance between clusters.
- t-SNE can be used to extract new features from data, in addition to EDA. Concatenate the new features with the original data.    

#### Practical notes
- Result heavily depends on hyperparams (perplexity).
    - It is good practice to use several projections with different perplexities (5-100).
- Due to the stochastic nature, t-SNE provides different projections even for the same data/hyperparams. Train and test sets should be projected together.
- t-SNE runs for a long time with a big number of features. It is common to do dimensionality reduction before projection (e.g., reduce 500 to 100).
- sklearn has an implementation of t-SNE, but the standalone python package 'tsne' is faster.
...
- Endemble modeling is combining different ML models to get a better prediction.
- The methods discussed in this section are encountered often in competitions.

### Averaging
- Example: two models that predict age. Plotting true age vs predicted age, we see that model 1 has $R^2$ = 0.9157, but doesn't do well past the age of 50. When age > 50, the average error is higher. Model 2 has $R^2$=0.9112, but does well when age > 50, and worse when age < 50.

#### Simple average
- Combining these two models with simple averaging results in $R^2$ = 0.9544.
    - Simple average: (model 1 + model 2)/2
    - This model does worse than the individual models on the areas where each individual model did really well, but overall has a lower error.

#### Weighted average
- Weighted average: (model1 \* 0.7 + model2 * 0.3)
- The resulting model has $R^2$ = 0.9478. The two individual models have similar predictive power, and it does not make sense to rely more on one than the other.

#### Conditional average
- Ideally, we would leverage the best parts of each model.
- In this example, the best condition would be to use model 1 when age < 50, and otherwise use model 2.
- Later sections describe ensemble methods that are very good at finding these conditions.

### Bagging
- Bagging is averaging slightly different versions of the same model as a means to improve accuracy.
- Random forests are an example of bagging.

#### Why use bagging? 
- Generally there are 2 main sources of errors in modeling:
    - 1. Errors due to **Bias** (underfitting)
    - 2. Errors due to **Variance** (overfitting)
- Models move from high bias & low variance to low bias & high variance as model complexity increases.
- Using slightly different models ensures that model does not reach very high variance and is more generalizable.

#### Bagging parameters
- Random seed.
- Row (sub) sampling or bootstrapping.
- Shuffling - some algorithms are sensitive to the data order.
- Column (sub) sampling.
- Model-specific parameters.
    - E.g., slightly different regularization parameters in linear models.
- Number of models (or bags).
  - Recommendation: start with 10 models.
- (Optionally) parallelism: in bagging, the models are independent, so can build independently.
...
for n in range(0,bags):
  ...
  bagged_prediction += preds # add predictions to bagged prediction
...
- Unlike in bagging, models are not independent.
- There are two main types of boosting: weight-based and residual-based.

#### Weight-based boosting
- Example: dataset with features x0, ... x3 and binary target y.
    - Build a classifier and generate predictions (probabilities).
    - Compute absolute error for each prediction. E.g., if y=1 and pred=0.8, then abs.error=0.2.
    - Create a new column with a weight = 1 + abs.error.
        - There are different ways to calculate the weight.
    - Fit a new model, but weigh each row by the calculated weight.
        - This can be interpreted as the number of times a certain row appears in the data.
    - Repeat the process, calculate new weights, etc.
- Parameters:
    - Learning rate (or shrinkage or eta).
        - predictionN = pred0\*eta + pred1\*eta + ... + predN\*eta
        - Ensures that not one model is dominating, and controls overfitting.
    - Number of estimators.
        - Has an inverse relationship with the learning rate. Larger number of estimators should have a smaller learning rate.
        - Tune this parameter with cross-validation.
        - Practical recommendation: start with 100 estimators, and try to find the optimal learning rate. Then, if increasing the number of estimators, decrease LR by the same factor. E.g., if n_estimators is now 200, learning rate should be divided by 2.
    - Input model: can be any model that accepts weights.
    - Sub boosting type:
        - AdaBoost - Good implementation in sklearn.
        - LogitBoost - Good implementation in Weka. Used for logistic regression.

#### Residual-based boosting
- Very successful and dominant type of algorithm in competitions.
- Example: dataset with features x0, ... x3 and binary target y, as before.
    - As before, fit model and generate predictions.
    - Calculate error, but not absolute error.
    - The error becomes the new target variable y.
    - Use the same original features to predict the new target variable.
    - To generate predictions for the original y, sum the predictions on original y and the residual y.
- Parameters:
  - Learning rate (or shrinkage or eta).
      - PredictionN = pred0 + pred1\*eta + ... + predN\*eta
      - Controls how the original prediction (pred0) moves towards the true y value.
      - A good way to prevent overfitting.
  - Number of estimators: same as for weighter averaging. More is better, but offset with LR.
  - Row (sub) sampling & column (sub) sampling.
      - These can also be used with weight-based boosting, but are used more frequently with residual-based.
  - Input model: works really well with decision trees, but theoretically can be other models.
  - Sub boosting type:
      - Fully gradient based: explained above in this section.
      - Dart: imposes a dropout mechanism to control the contribution of the trees. Concept derived from deep learning. Randomly exclude trees when generating new trees. Works as regularization.

#### Residual-based implementations
- XGBoost
- LightGBM
- H2O's GBM: can handle categorical variables out-of-the-box. Comes with grid search.
- Catboost: comes with a strong initial set of parameters, and handles categorical variables out-of-the-box.
- GBM (sklearn): can put any sklearn estimator as the base.

### Stacking
- Very popular form of ensembling. Most competitions use a form of stacking in the end.
- Means making predictions using a number of models on a hold-out set to create a new dataset, and then using a different (Meta) model to train on this predictions-dataset.
- The meta model does not use the original input data.
...
- 4. Use predictions from (3) as the inputs to train a higher-level learner (metamodel).

#### Another explanation
- Consider datasets A, B, and C. Target variable (y) is known for A & B.
- A is the training data set, B is validation, C is the test set for final predictions.
- Train algorithm 0 on A, then make predictions on B and C at the same time. Save these predictions as columns of new datasets B1 and C1 (one column for data B, and one column for data C).
- Train algorithm 1 on A, make predictions on B and C, and stack with previous columns in B1 and B2.
- Repeat for algorithm 2.
- This results in dataset B1 having 3 feature columns and the original target y, and dataset C1 having 3 feature columns (and no predictions, since they are unknown).
- Train algorithm 3 (the meta-learner) on B1, and make predictions on C1.
...
- Example of 2 models predicting age, with a split at age = 50.
- Applying stacking results in a model that performs similar to the model made from best parts of model 1 and 2. The stacked model is slightly less accurate at around age = 50, but is still able to identify the split at 50.

#### Things to be mindful of
- Time series data: create the data splits so that train is in the past, and validation is in the future. Ideally, the test data is also in the future.
- Diversity is as important as single-model performance. Even weak models may bring new information that helps the meta learner.
- Diversity may come from:
    - Different algorithms. E.g., a linear model will focus on linear relationships.
    - Different input features. Run same model on different features or transformations.
- Performance plateaus after some number of models.
    - There will be a significant uplift at the start.
- Meta model is normally modest.
    - Leave the main work to the base learners.
...
- This method extends stacking to have multiple layers of meta-learners.
- These methods are criticized for having no commercial value and being computationally expensive.
    - What is expensive today may not be expensive tomorrow.
    - Very deep ensembles are not always necessary, but small ensembles do help. This can add value to business. It depends on how fast predictions are needed, how much computing power is available, etc.
    - These are competitions, and show the theoretical best.
...
- StackNet architecture is very similar to that of neural networks.

#### How to train
- Cannot use backprop, since not all models are differentiable.
- Stacking is used in place of backprop to link each model/node with the target.
- Use k-fold for data splitting (k can also be tuned).
    - Example for k = 4:
        - Split the train data into 4 equal parts.
        - Create an empty pred vector with same number of rows as original train set.
        - For fold 1, train a base model on folds 2-4, and make predictions on fold 1. Fill pred vector with these predictions in the appropriate rows.
        - Repeat for fold 2, then 3, then 4, to fill the entire pred vector.
        - Then use the whole original train set to fit another model and make predictions on test.
        - Generate another empty vector, stack it next to the previous one, and repeat the above steps.
        - After generating several empty vectors and filling them, this is now the new training dataset.
- Instead of epochs, this method has different connections. This is similar in spirit to epochs, as it changes the weights through revisiting.
- Can also use the original dataset for the final predictions. This works well if there are not many models.
- This is similar to target/mean encoding, where features are replaced with holdout predictions of the target variable.

### Ensembling tips and tricks

#### 1st level tips (base learners)
- Diversity based on algorithms. The following is a useful architecture:
    - 2-3 GBT (LightGBM, XGBoost, H2O, Catboost). Use different libraries/implementations. Create a model with low depth, a model with mid depth, and one with high depth; tune params so that performance is similar across these models.
    - 2-3 neural nets. Again, create a model with 1 hidden layer, another model with 2-3, etc.
    - 1-2 ExtraTrees/RandomForest.
    - 1-2 linear models as in logistic/ridge regression, linear svm.
    - 1-2 knn models (sklearn). These are not good on their own, but add value in a stacking context.
...
- Diversity based on input data:
    ...
    - Numerical features: outliers, binning, derivatives (for smoothing), percentiles, scaling.
    - Interactions: pairwise, groupby, unsupervised (kmeans, SVD, PCA).

#### Other levels
- Simpler (or shallower) algorithms:
    - GBT with small depth (2-3)
    - Linear models with high regularization
    - ExtraTrees
    - Shallow NN (with 1 hidden layer)
    - kNN with Bray-Curtis distance
    - Brute fore a search for best linear weights based on cv
- Feature engineering:
    - pairwise differences between meta features
    - row-wise statistics like averages or stds
    - standard feature selection techniques
- Rule of thumb: for every 7.5 models in previous level, add 1 in the next layer.
- Be mindful of target leakage. Might be introduced despite the holdout mechanism.
    - Control this by selecting the right k. For high k, each model uses more training data, and might not generalize well.
...
- Supports many prominent tools (XGBoost, lightGBM, H2O, keras, ...).
- Can run classifiers in regression and vice versa.
    - E.g. instead of predicting age, predict if this person is over 50 years old.
    - This tends to work well.
- Has several top 10 solutions in competitions.
- Amazon competition has a good example kernel: sub_70_30.7z
- The parameters section lists which params are most important.
...
- We may often encounter a situation where scheme f) is not applicable, especially with a limited amount of data.
- For example, when we have only years 2014, 2015, 2016 in train and we need to predict for a whole year 2017 in test.
- In such cases, scheme c) could be of help, but with one constraint: KFold split should be done with the respect to the time component. For example, in case of data with several years we would treat each year as a fold.
...
#### Problem formulation
- Measure relevance of the search results, given the queries and resulting product descriptions from e-commerce sites.
- The task is to evaluate the accuracy of their search algorithms.
- The challenge is to predict the relevance score of the given query and product description.

#### Assessor UI
- The interface has a query and an item.
- Assessors are asked to give each query-product pair a score of 1 (no match) to 4 (complete match).
- The training data has a median and variance of these scores.

#### Data
- The dataset consists of: id, query, product_title, product_description, median_relevance, relevance_variance.
-  To ensure that the algorithm is robust enough to handle any HTML snippets
 in the real world, the data provided in the program description field is raw and
 sometimes contain information that is relevant to the product. E.g., object ID.
- To discourage hand labeling, the actual test set was augmented with extra data. This additional data is ignored when private and public LB scores were calculated. Competitors have no idea which objects were used to calculate the scores.
- 10,000 samples in train, and 10,000 in test.

#### Competition metric
- Quadratic weighted kappa.
- Typical value range: from 0 (random) to 1 (complete agreement between two ratings).
- May go below 0.
- Has properties both to standard classification loss and regression loss. Larger distance between true and predicted ratings leads to larger penalty.

#### Our solution
##### Text features: similarities
- We have 3 text fields: a query, a title, and a description.
- We can calulate various measures of similarity. That is, for (query, title) and (query, description) pairs we calculated:
...
- The above list is a standard set of features that should be used for similar tasks. These features are considered as a baseline.

##### Text features: symbolic n-grams
- Each letter is interpreted as a separate word.
- We used symbolic n-grams for 1 letter to 5 letters.
- Example: for a text input of "once upon a time", generate the following n-grams: 'Once_', 'nce_u', 'ce_up', 'e_up', etc.
- After getting a large sparse matrix of n-grams, apply SVD and keep the first 300 components as features.

##### 3 important data properties
- Observations: queries are very short, the number of unique queries is 261, and queries are the same in train and test.
- Use these observations to build extended versions of queries as follows:
    - For each query, get the most relevant corresponding items (those with relevance = 4), join all words from the product title, and take 10 most popular words.
    - Example: query_original = "bridal shower decorations"; query_extended = "shower bridal banner person decor mr 192 30 36 new".
- Use the extended queries to build features mentioned earlier.
- This trick is applicable only within this contest framework, since queries in the test set are *exactly* the same as in train.

##### Per-query models
- The fact that queries are the same in train and test gives us the opportunity to split our problem into many small subtasks.
- Specifically, for each query, you can build a separate model, that will only predict relevance of corresponding items.
- Again, this cannot be applied in real life.
- So, for each unique query, we get corresponding samples, calculate word2vec similarities, bag-of-words representation, and fit a random forest classifier. This results in 261 models; the predictions of these models were used as features in the final ensemble model.

##### Sample weighting
- We are given the variance of scores for each (query, product) pair.
- The variance shows if the ratings are inconsistent or not. Large variance means low confidence about the label.
- Intuitively, if variance is large, this observation should have a a smaller weight.
- One way to do this is to assign weights based on variances: $w = \frac{1}{1+\sigma}$.
    - This approach worked well and was used in the final solution.
- Another way is to restore individual scores using median and variance statistics. It is possible to do this assuming that there are 3 raters.
    - Example: if median = 3.0 and $\sigma$ = 0.66, then the scores are probably [2,3,4].
    - Using this approach, can generate 3 new observations from each original sample.
    - This approach took longer to train and resulted in almost no gain.

##### Bumper features
- For quardratic kappa, penalty for the error is not symmetric.
- We took this into account, adding several artificially created binary delimiters between classes as features.
- In other words, we train the classifiers to answer these questions:
    - "Is it true that the target class number is greater than 1?"
    - "Is it true that the target class number is greater than 2?"
    - "Is it true that the target class number is greater than 3?"
- These features are called "bumpers" since they act like separators between classes.
- Build these features in out-of-fold fashion, similar to constructing first-level predictions in stacking.
- These features were very useful for the final solution.

##### Ensemble
- We build several models on different subsets of features. Some were relatively simple, like SVM, and some were quite complex.
- There are many first-stage models mixed up with second-stage random forest.
- Each participant in the team made their own model. For the final submission, the models were combined linearly.

##### Kappa optimization
- The metric has properties of classification (it's necessary to predict the class) and regression (larger deviation from correct answer gets more penalty).
- We built the model as regression, but needed a way to turn real-valued predictions into classes.
- We varied thresholds for every class and selected the best ones using cross-validation.
- This significantly increased the score.

##### Conclusion - most important parts of our solution
- Symbolic n-grams
...
- This section describes the last part of the solution - stacking and ensemble.
- The solution consists of several levels, starting with the features.

#### Task description
- Determine whether to send a direct mail piece to a customer.
- Binary classification task with AUC metric.
- Train data: 145000 samples, 2000 anonymized features.

#### Level 0 - Feature packs
- These are features derived and discovered during EDA.
- Basic dataset: basic data cleaning and feature engineering.
- Processed dataset: advanced data cleaning and feature engineering.
    - Feature interactions based on scatter plots and correlations, etc.
- Mean-encoded dataset: projecting features into homogeneous space.
- KNN dataset: distance features on mean-encoded data.
    - Distance to closest object of class 0.
    - How many of the 10 nearest neighbors belong to class 1.

#### Level 1
##### Out-of-fold predictions - xgboost
- Several different GBDT models and one NN model. The main idea is that meta features should be diverse.
    - We used both different sets of params and different sets of features for the xgboost models.
    - For NN, preprocess the features with common scalers (StandardScaler, Ranks, Power).

##### Pairwise differences
- After producing metafeatures with GBDT and NN, calculate pair-wise differences to help next-level models. This is a trick to force the model to utilize differences in the first-level model predictions.
- Added two datasets of features based on nearest neighbors:
    - One directly from Level 0.
    - One on the mean-encoded dataset to power of 1/2.

##### Level 2 - classifiers
- Train simple classifiers (meta models) on the out-of-fold predictions.
- Models: GBTD, NN, RF, Logistic Regression.

##### Level 3
- Linear combination of the Level 2 models.
...
#### Problem description
- Given ~20000 program executables (.5TB), each given in two forms:
    - HEX dump (row format)
    - IDA disassembly
        - IDA (interactive disassembly) converts low-level sequence of bytes from HEX dump to a sequence of assembler commands.
- Test-train split was 50-50.
- Task: classify malware into 9 families
- Evaluation metric: LogLoss.
    - The final logloss for this task was ~0.003, which is extremely low for a 9-class task.

##### Quick check for leaks
- Data was provided in .7z archives. File names looked like hash values.
- There was no useful information found in the file names, since the split was random.

#### Feature extraction
##### Data exploration
- Each executable is associated with two files. Using the two sizes as features gets 12% error rate.
- Get counts of bytes in each row of the hex dump. This generates 257 features (256 byte values and 1 special symbol). Using these features gets 1.3% error rate.
- Extract information from the disassembly file using regex. Get counts of "sys call" ("api call" in Windows). This gets 2.3% error rate.
- Count assembler operators. This gets 1.5% error rate.
- Count lines which start with ".text", ".data", etc. This gets 0.8% error rate.
- Incorporating all the above features results in 0.46% error rate.

##### Features: n-grams
- Tried 2-, 4-, and 10-grams with feature selection.

##### Entropy
- Entropy extraction: compute entropy for a sliding window of the byte sequence.
- Entropy is expected to be high if the data is encrypted and compressed. Entropy should be low if the data is structured.
- The motivation for this approach is that some malware is stored in encrypted format when injected into normal executables. When the program starts, the malware extracts itself in the background and then executes. Entropy features can help identify detect encrypted regions in executable, and thus detect some types of malware.
- Feature generation:
    - The entropy sequence is of variable length, and the features cannot be used as-is.
    - Generate features by taking statistics of the entropy distribution: mean, median, min, max, etc.
    - Compute percentiles, inverse percentiles.
    - Compute similar features from entropy changes (apply diff first, then compute statistics).

##### Strings
- It was possible to extract strings from the hex dump by looking for printable sequences with "\0" at the end.
- The strings themselves were not used as features.
- Compute string length distribution in the files.
- Extract similar statistics as above (quantiles, percentiles, inverse percentiles, etc.)

#### Feature processing and selection
- The above steps generate many features, but most of them are probably useless.
- Try different feature selection methods and transformation techniques.

##### Feature transformations: NMF vs PCA
- Both these methods try to factorize an object-feature matrix into the product of two low-rank matrices. This can be thought of as finding a small number of basis vectors in feature space so that every object can be approximated by a linear combination of those vectors. The coefficients of this approximation can be treated as new features for each object.
- NMF requires all the components of the low-rank matrices to be non-negative.
- NMF turned out better for using in random forest models, since non-negativity is essential, and in this case we were working with count data (non-negative by nature).
- An easy way to get more features is to take a log transform (log(X+1)) of the data and compute NMF on the transformed data. This trick implicitly changes the loss that NMF optimizes from MSE to RMSLE (RMSLE is MSE in log space). Since the loss is different, the optimization process focuses on different things than before, and produces different features.

##### Feature selection: 4-grams
- We used a small pipeline to select 4-gram features:
    - Omit rare features
    - Apply linear SVM + L1-penalty
    - Threshold random forest feature importances to get final features.
- This reduces the number of features from (256+1)^4 to 131, and leads to an error rate of 0.48%.

##### Feature selection: 10-grams
- The pipeline for 10-grams is more complicated.
    - Use the hashing trick to reduce dimensionality of the original features.
    - Omit rare features
    - Apply linear SVM + L1-penalty
    - Threshold random forest feature importances to get final features.
- This reduces the number of features from (256+1)^10 to 861, and leads to an error rate of 0.32%.
- The main problem with feature selection described here is that the 10-grams features were generated independently from the rest of the features. After adding these 10-gram features to the rest and doing selection, we may end up with features that are really good, but contain redundant information. To *improve* the previous features by adding 10-grams, follow this process:
    - 1. Fit a classifier on the features X (all the features) and obtain OOF predictions for the train set.
    ...
    - 3. Fit a model on the new dataset and select best-performing features. 14 10-gram hashes were selected in this case.
    - Adding these 14 features resulted in increased performance.
    - This method can lead to severe overfitting, but worked out nicely in this case.

#### Models
- Did not use stacking in this competition (stacking became popular after this competition).
- First used Random Forest. RF needed to be calibrated for log loss manually, so instead moved to GBDT.
- GBTD (XGBoost) was used for all modeling.
- Each person on the team had their own features, models, and parameters. The final solution was a combination.
- Bagging worked well with boosting.
    - Took a special approach to bagging. Concatenate train set with a 7x larger set of objects sampled from train with replacement. Use the resulting data set to train xgboost.
- Use about half of all features for every tree.
- Average over ~20 runs to account for randomness.

#### Tricks
##### Pseudo labeling
- The CV accuracy was quite high (>0.997). We also know that more data is better. So, decided to use testing data for training the models. To accomplish this, need labels for the test data. Use either predicted class, or sample the label from the predicted class distribution.
- To perform cross-validation with pseudo labels: split the data into folds, and before training, concatenate the fold with test data. Then, switch folds and repeat.
- To get predictions for test set, divide the test set into folds and concat train set to each fold of the test set.

##### Per-class weighting
- Usually, models are mixed linearly: $y_{w} = {\alpha}y_1 + (1-\alpha)y_2$.
- In this competition, we tuned coefficients for each class separately: $y^i_{w} = {\alpha}^iy^i_1 + (1-\alpha^i)y_2^i$. This is similar to stacking with linear model of a special kind at the second level.

##### Source code
- The code is available at github: https://github.com/geffy/kaggle-malware.



### Walmart: Trip Type Classification
#### Problem statement
- Given data about purchases, classify it into 38 distinct trip types.
- Predict trip type for visit number, and not for each row in the dataset.

#### Data format
- 647000 rows, 95000 visits in the train data.
- Features: TripType, VisitNumber, Weekday, Upc, ScanCount, DepartmentDescription, FinelineNumber.
    - Upc: exact id of the item.
    - ScanCount: number of items purchased. "-1" represents a return.
    - DepartmentDescription: 68 unique values.
    - FinelineNumber: more refined category for an item (~5000 categories).
- In the default data item, several rows might belong to the same visit number. Using the data as-is does not capture important interactions between the items.
    - E.g., TripType may have the number 26 if more than half of its items are from the "paint and accessories" category. But, there is no accounting for the interactions between items.
- Shaping the data so that each row is a complete visit will allow to capture interactions.
    - After converting to this format, the items purchased in a visit can be treated as words in a text, and some of the usual text processing techniques apply (e.g., TF-IDF).

#### Models
- Tree models and neural networks capture interactions better than linear models. However, linear models are useful for generating meta-features.

#### Feature engineering
- One interesting observation is that one FinelineNumber can belong to multiple department descriptions. This means that FinelineNumber is not as detailed as originally thought.
- Another feature generation idea was connected to the time structure of the data. The data appears to be sorted in chronological order and span 31 days, but the train-test split was not time-based. This makes it possible to derive features like day number in the dataset, number of visit in day, and total amount of visits in day.



### Acquire Valued Shoppers Challenge, part 1
#### Background
- Build a recommender to recommend items to customers.
- The solution was focused on really understanding the problem and properly validating.

#### Problem statement
- 310,000 shoppers (160K train, 150k test)
- 350,000,000 transactions (for 1 year+ for each shopper)
- 37 offers (coupons) incentivizing purchase (24 offers in train data).
- The data did not have a 'product' column, but this could be inferred (from brand, category and company)
- For each customer, there is a 1-year timeline of visits/purchases, and then the coupon. All transactional history stops after the coupon. It is known if the customer redeemed the coupon. For training data, there is another flag of whether the customer purchased that item *again* afterwards.
- Optimize AUC for whether the shopper will buy again.

#### "Challenges" of this challenge
- Large dataset
- No features given; had to create the features yourself.
- Irregular testing environment:
    - Train and test data had different customers (no overlap).
    - Most offers apper only in test or in train (minimal overlap).
- Test data may we well in the future.
- Focused on acquisition. Limited history of customer and offer (coldstart problem).
- Offers' propensity to buy varies significantly.

#### Handling big data
- Use indexing. The dataset was already sorted by customer and time. Parse through the transactions data. For each customer, create a new file with all of that customer's history.
- To generate features for a specific customer, just access their file. This can be done in parallel.
- Also create a file for each category, brand, company.

#### Handling irregularity
- Explored different cross validation.
- Stratified k-fold based on offer (so that each offer is represented equally). This had good internal results, but doesn't really work since test and train data have *different* offers.
- Leave-one-offer-out. This gave better results. However, while AUC is really good when picking between 2 offers, it is not as good when picking between all 37 offers.
- Combine the 2-offer AUC and multi-offer AUC into one new metric to optimize. This metric was the closest in results on the test set.

### Acquire Valued Shoppers Challenge, part 2
#### Strategies for recommenders
##### Content-based approach
- Recommendations are made on characteristics of the product.
- Assumption: a customer who has bought from the same company, brand, and category, will have a higher chance to buy the products once offered.
- Features exploiting product hierarchy versus customer and time.
    - Product sales can be seasonal (e.g. ice cream in summer).
    - Define time intervals (last 30, 60, 90, 120, 180, 360 days) that the customer has purchased from combination of same company, brand, or category.
        - E.g. times bought same category & brand in the last 30 days; times bought the same category & company in last 30 to 60 days, etc.
- Features selected through forward cross validation: add 1 feature at a time.
- Big values were capped, and missing values were replaced with -1. Many of the missing values were produced by the generated features.
- Model using Ridge Regression on the actual repeat count (not on the binary repeated or not class). This gives stronger information.
- This scored 0.610+ on test data (near top-10 on LB).

##### Collaborative filtering approach
- Make recommendations based on similar customers' preferences, *not* on the item.
- Would the customers have bought the product had they not received the offer?
- Made a model for every offer in train and test.
- The target variable is the natural log of the times a customer bought the product 90 days BEFORE receiving the coupon. This ignores the coupon completely. This identifies customers that like the product regardless of the offer.
- Features were focused on user activity:
    - Counts of popular categories, brands, customers.
    - Used Restricted Boltzmann Machines to summarize purchase activity on least popular categories.
        - Inspired by Hinton.
        - This is an unsupervised approach.
    - Other features: average amount purchase, total visits, distinct brands/categories/companies.
    - Total discounts/returns, visits on weekends, spend in weekends.
- Models using GBM (from sklearn) on the log of counts of how many times the product was bought in 90 days before the offer.
- This scored 0.616+ on the test.

##### Hybrid approach
- Both models were trained on different targets. This is tough to combine normally.
- However, irrespective of the scores, the distributions looked similar.
- This gave us the idea that since AUC cares about rank, to transform the scores into rank.
- So, convert the scores to rank and merge the two approaches by averaging.
- This scored 0.626 on the test.
