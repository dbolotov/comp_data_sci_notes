...
##### R-squared metric
$$R^2 = 1 - \frac{\frac{1}{N}\Sigma_{i=1}^N(y_i-\hat{y_i})^2}{\frac{1}{N}\Sigma_{i=1}^N(y_i-\bar{y_i})^2}= 1 - \frac{MSE}{\frac{1}{N}\Sigma_{i=1}^N(y_i-\bar{y_i})^2},$$
$$\bar{y} = \frac{1}{N}\Sigma_{i=1}^Ny_i$$
...
- Relative error preference can be expressed with MSPE (mean square percentage error) and MAPE (mean absolute percentage error):
    - $MSPE = \frac{100\%}{N}\Sigma_{i=1}^N(\frac{y_i-\hat{y_i}}{y_i})^2$
...
- $RMSLE = \sqrt{\frac{1}{N}\Sigma_{i=1}^N(log(y_i+1)-log(\hat{y_i}+1))^2} = RMSE(log(y_i+1),log(\hat{y_i}+1)) = \sqrt{MSE(log(y_i+1),log(\hat{y_i}+1))}$
...
#### RMSLE baseline
- $RMSLE = \sqrt{\frac{1}{N}\Sigma_{i=1}^N(log(y_i+1)-log(\alpha+1))^2} = RMSE(log(y_i+1),log(\alpha+1)) = \sqrt{MSE(log(y_i+1),log(\alpha+1))}$
...
- MSPE and MAPE are biased towards smaller targets, as they assign higher weight to objest with smaller targets.
...
#### Accuracy score
- Accuracy = $\frac{1}{N}\Sigma_{i=1}^N[\hat{y}=y_i]$
...
- *Binary*: LogLoss = $-\frac{1}{N}\Sigma_{i=1}^Ny_ilog(\hat{y_i}) + (1-y_i)log(1-\hat{y_i}), y_i \in R, \hat{y_i} \in R$
...
- *Multiclass*: LogLoss = $-\frac{1}{N}\Sigma_{i=1}^N\Sigma_{i=1}^Ly_{il}log(\hat{y_{il}}), y_i \in R^L, \hat{y_i} \in R^L$
...
- *In practice*: LogLoss = $-\frac{1}{N}\Sigma_{i=1}^N\Sigma_{i=1}^Ly_{il}log(min(max(\hat{y_{il}},10^{-15}),1-10^{-15}))$
...
- Baseline:
    - LogLoss = $-\frac{1}{N}\Sigma_{i=1}^Ny_ilog(\alpha) + (1-y_i)log(1-\alpha)$
...
- $p_e$ can be computed analytically: $p_e = \frac{1}{N^2}\Sigma_kn_{k1}n_{k2}$.
...
- Weighted error = $\frac{1}{const}\Sigma_{i,j}C_{i,j}W_{i,j}$
...
- Sample weights:
    - MSPE: $w_i = \frac{1/y_i^2}{\Sigma_{i=1}^N1/y_i^2}$
    - MAPE: $w_i = \frac{1/y_i}{\Sigma_{i=1}^N1/y}$
